  ...
  return (
    <div className="mt-6">
      ...
      <div className="hidden md:block rounded-md border border-zinc-200 bg-white p-3 shadow-sm sm:p-4">
        <div className="w-full overflow-x-auto">
          <table className="w-full table-auto border-collapse">
            ...
            <tbody>
              {students.map((s) => (
                <tr key={s.id ?? s.studentId} className="border-t bg-white">
                  <td className="py-3 pr-4 text-sm font-medium text-zinc-600">{s.name}</td>
                  <td className="py-3 pr-4 text-sm text-zinc-600">{s.studentId ?? s.id}</td>
      ...
      <div className="md:hidden space-y-3">
        {students.map((s) => (
          <div key={s.id ?? s.studentId} className="rounded-md border border-zinc-200 bg-white p-3 shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-sm font-medium text-zinc-900">{s.name}</div>
                {(s.studentId ?? s.id) && <div className="mt-1 text-xs text-zinc-600">ID: {s.studentId ?? s.id}</div>}
